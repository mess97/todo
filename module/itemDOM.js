...
export const getItemDOM = (inputValue) => {
  ...
  checkInpt.onclick = () => {
    ...
    if (getLocalInfo[checkInpt.id].state == "false") {
      getLocalInfo[checkInpt.id].state = "true";
      taskItem.classList.add("list-check");
      itemTitle.classList.add("title-check");
    } else {
      getLocalInfo[checkInpt.id].state = "false";
      taskItem.classList.remove("list-check");
      itemTitle.classList.remove("title-check");
    }
  ...
  taskDelete.onclick = () => {
    ...
    // const newInfo = getLocalInfo.filter(
    //   (item) => item.title !== inputValue.title
    // );

    let newInfo = getLocalInfo.filter((x) => x.title !== inputValue.title);
    localStorage.setItem(TODO_COOKIE_KEY, JSON.stringify(newInfo));
